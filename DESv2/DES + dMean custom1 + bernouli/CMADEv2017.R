CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  cmu         <- controlParam("cmu", 0.3*lambda/N^2)
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
      ...
      prb <- rep(c1+cmu,limit)
      prb <- prb ^ (seq_along(prb)-1)
      prb <- prb*(1 - c1 - cmu)
      historySample <- sample(1:limit,lambda,prob=prb, T)
      historySample2 <- sample(1:limit,lambda,prob=prb, T)
