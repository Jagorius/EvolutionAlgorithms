CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  while (counteval < budget) {
      ...
      # Save normalized population and Ft in the history buffer      
      pp=normalizePop(population)
      Ft=pp$Ft
      meanPop=pp$means

      ## Select best 'mu' individuals of population
      selection       <- order(fitness)[1:mu]
      history[,,histHead] <- pp$population[,selection]
      meanSel=rowMeans(population[,selection])
      
      ## Write to buffers
      step <- (meanSel - meanPop) / Ft
      steps$write(step)
      ...
      if (iter > pathLength-1 && (sum(step == 0) == 0) && counterRepaired<0.1*lambda) {
        ...
        Ft <- calculateFt(steps, N, lambda, pathLength, Ft, c_Ft, pathRatio, 2.78, mueff)
...
#  return (rnorm(1,g_sd)) 
  ...
  return (currentFt * exp(c_Ft * (refVal / (totalPath / directPath)-1)))  
...
normalizePop<-function(population)
{
   means=rowMeans(population)
   
   ss=sum(rowMeans((population-means)^2))
   Ft=sqrt(ss/(1-1/dim(population)[2]))
   return(list(means=means,Ft=Ft,population=(population-means)/Ft))
}
