  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
        ...
        if (counteval < 0.85*budget)
        {
          ...
          # Stop if range of best value for last 10+30*N/lambda generations is smaller than tol
          if(iter > round(10+30*N/lambda)){
            bestRange <- range(bestHist[iter-round(10+30*N/lambda):iter])
            if(bestRange[2]-bestRange[1] < tol)
              stoptol=T
          }
        }        
    }
