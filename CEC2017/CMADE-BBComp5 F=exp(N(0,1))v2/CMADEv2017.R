CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
        ...
        history[[histHead]] <- (selectedPoints * histNorm) %*% diag(Ft_vec[selection])
        ...
        ## Write to buffers
        step <- (newMean - oldMean) / mean(Ft_vec)
        steps$write(step)

        ## Update Ft
        FtHistory[histHead] = 1
        oldFt <- Ft
        ...
        ## Price method of calculating Ft
        Ft_vec <- replicate(lambda, exp(rnorm(1)))

        ## New population
        population <- newMean + diffs %*% diag(Ft_vec) + tol*rnorm(diffs)/chiN
