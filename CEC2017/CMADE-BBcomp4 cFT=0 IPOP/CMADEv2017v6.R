CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  #initlambda  <- controlParam("lambda", round(4+9*log(N)))           ## Population starting size
  if(N == 10)
    initlambda <- 50
  else if(N == 30)
    initlambda <- 70
  ...
  initmu      <- controlParam("mu", floor(lambda/2))                  
  mu          <- initmu                                               ## Selection size
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
        ...
        if (counteval < 0.85*budget)
        {
          # Stop if the only thing that affects the population is the noise
          if(mean(apply(population,1,sd)) < tol)
            stoptol=T
         
          popRange = range(fitness)
          if(popRange[2]-popRange[1] < tol)
            stoptol=T
          
        }
        
    }
    # Increasing size of population with bounding mechanizm
    lambda <- lambda*2
    if(lambda > maxlambda)
      lambda  <- initlambda
