...
#  refPoint<-rnorm(N,lower,upper)
#  population <- replicate(lambda, refPoint+rnorm(N))
  population <- replicate(lambda, runif(N,lower,upper))
#  population <- 2*(population-lower)/(upper-lower)-1
#  Ft<-(upper-lower)/2
...
#        diffs[,i] <- sqrt(1-c_pc) *((x1 - x2) + rnorm(1)*step*chiN) +
