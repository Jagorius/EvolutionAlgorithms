CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  while (counteval < budget) {
     ...
     if(counterRepaired>0)
      ...
      ## New population
      population <- newMean + rnorm(1,Ft) * diffs

      ## Values can not exceed the maximum value supported by the interpreter
      population [is.na(population)] <- .Machine$double.xmax
      population [is.infinite(population)] <- .Machine$double.xmax
