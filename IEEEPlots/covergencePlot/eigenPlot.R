eigenplotDES<- function(){
  ...
  options(scipen=3)                      #ylim=c(1e-7,1e1)
...
eigenCMAESwithfitness <- function(){
  source('C:/Users/JS/Desktop/Doktorat/EvolutionAlgorithms/IEEEPlots/CMAES.R')
  N <- 10
  Iters <- 1
  L = 4*N
  ...
  for (i in 1:Iters){
    ...
    resCMAES <- cma_es(rep(0,N),
                       ...
                       lower=-10^100, upper=10^100,
                       control=list("diag.eigen"=TRUE,"diag.eigen"=TRUE,"diag.bestVal"=TRUE,"budget"=100000,"lambda"=L)
    )
    eigenLog[[length(eigenLog)+1]] <- abs(resCMAES$diagnostic$eigen)
  }
  resCMAES <<- resCMAES
...
eigenDESwithfitness <- function(){
  ...
  for (i in 1:Iters){
    ...
    lower=-10^100, upper=10^100,
    control=list("budget"=100000 ,"diag.pop"=TRUE,"diag.eigen"=TRUE,"diag.bestVal"=TRUE)
    )
    eigenLog[[length(eigenLog)+1]] <- abs(resDES$diagnostic$eigen)
  }
  resDES <<- resDES
...
eigenCMADEwithfitness<- function(){
  source('C:/Users/JS/Desktop/Doktorat/EvolutionAlgorithms/DESv2/CMA-DE/DESv2017.R')
  ...
  for (i in 1:Iters){
    ...
    lower=-10^100, upper=10^100,
    control=list("budget"=40000,"diag.pop"=TRUE,"diag.eigen"=TRUE,"diag.bestVal"=TRUE)
    )
    eigenLog[[length(eigenLog)+1]] <- abs(resDES$diagnostic$eigen)
  }
  resCMADE<<-resDES
