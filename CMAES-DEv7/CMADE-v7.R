CMADE7 <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  initlambda    <- controlParam("lambda", floor(((1-tanh(0.2*(N-10)))*(3+sqrt(N)/4)+(1+tanh(0.2*(N-10)))*(3.5+sqrt(N)/1.5))*N/2))#*N#*(0.8*(N<=4)+(N>4))))                          ## Population size
  #ldef <- max(min(floor((3.5+sqrt(N/2))*N),budget/260),2*(N+1))
  #  ldef <- max(min(floor((3.5+sqrt(N))*N),budget/260),2*(N+1))
  
  #  initlambda    <- controlParam("lambda", ldef)#floor(((1-tanh(0.2*(N-10)))*(3+sqrt(N)/4)+(1+tanh(0.2*(N-10)))*(3.5+sqrt(N)/1))*N/2))#*N#*(0.8*(N<=4)+(N>4))))                          ## Population size
  lambda <- initlambda
  #  lambda      <- controlParam("lambda", floor((3.5+sqrt(N)/3)*N))#*N#*(0.8*(N<=4)+(N>4))))                          ## Population size
  #  initlambda  <- controlParam("lambda", floor((3.5+sqrt(N)/3)*N))#*N*(0.8*(N<=4)+(N>4))))                          ## Population size
  #  if (budget/lambda<100)
  #     initlambda=lambda = budget/100
  #  if (lambda<=N+1)
  #     initlambda=lambda = N+2
  ...
  histSize    <- controlParam("history", 6+ceiling(3*sqrt(N)))#5*log(N)/log(2))                     ## Size of the window of history - the step length history
  ...
  while( counteval < budget && best.fit > stopfitness)
  {
    ...
    #    population<- replicate(lambda,runif(N,lower+(upper-lower)*0.25,upper-(upper-lower)*0.25))
    ...
    while (counteval < budget && !stoptol) {
      ...
      #print(c(counteval,restart.length,best.fit, norm(pc)*Ft, Ft)) 
