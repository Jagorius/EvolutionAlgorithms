...
void searchAlgorithm::evaluatePopulation(const vector<Individual> &pop, vector<Fitness> &fitness) {
  ...
  for (int i = 0; i < pop_size; i++) {
	...
	for (int j = 0; j < l_problem_size; j++) {
		 if (pop[i][j] < l_min_region)  repaired[j] = l_min_region + ((int)(l_min_region - pop[i][j])) % ((int)(l_max_region - l_min_region));
		 if (pop[i][j] > l_max_region)  repaired[j] = l_max_region - ((int)(pop[i][j] -l_max_region )) % ((int)(l_max_region - l_min_region ));
	}
