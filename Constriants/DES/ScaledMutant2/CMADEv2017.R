CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  ScaledMutant2 <- function(x, baseVector){
    indxs = which( x < lower )

    if(length(indxs)>0){
      #if(any(newPop[ indxs, i ]-baseVectors[ indxs, i ]==0))
      #  browse()

      if( any( x[ indxs ]-baseVector[ indxs]==0) ){
        numericErrIndxs = which( x[ indxs ]-baseVector[ indxs ]==0 )
        x[ indxs[numericErrIndxs] ] = baseVector[ indxs[numericErrIndxs] ]
        indxs = which( x < lower )
        if(length(indxs)>0){
          alphas= (lower[indxs] - baseVector[ indxs]) /  (x[ indxs ]-baseVector[ indxs ])
          alpha = min(1, alphas)
          x = alpha* x+(1-alpha)*baseVector
        }
      }else{
        alphas= (lower[indxs] - baseVector[ indxs ]) /  (x[ indxs]-baseVector[ indxs ])
        alpha = min(1, alphas)
    ...
    if(length(indxs)>0){
      ...
      if( any( x[ indxs ]-baseVector[ indxs ]==0) ){
        ...
        indxs = which( x > upper )
        if(length(indxs)>0){
          alphas= (upper[indxs] - baseVector[ indxs ]) /  (x[ indxs ]-baseVector[ indxs ])
          alpha = min(1, alphas)
          x = alpha* x+(1-alpha)*baseVector
        }
      }else{
  ...
  while( counteval < budget){
    ...
    populationRepaired <- apply(population,2,ScaledMutant2,baseVector=newMean)
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(ScaledMutant2(newMean,oldMean)))
      ...
      cumMeanRepaired <-ScaledMutant2(cumMean,cumMean_old)
