CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  RandBase <- function(x, baseVector){
    indxs = which( x < lower )
    x[ indxs] = runif(length(indxs), lower[indxs], baseVector[indxs])
    indxs = which( x > upper )
    x[ indxs] = runif(length(indxs), baseVector[indxs], upper[indxs] )
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(RandBase(newMean,oldMean)))
      ...
      cumMeanRepaired <-RandBase(cumMean,cumMean_old)
