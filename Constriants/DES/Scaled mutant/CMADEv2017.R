CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  ScaledMutant <- function(x){
    indxs = which( x < lower )
    alphas= lower[indxs]/x[ indxs ]
    alpha = min(1, alphas)
    x = alpha* x
    indxs = which( x > upper )
    alphas= upper[indxs]/x[ indxs ]
    alpha = min(1, alphas)
    x = alpha* x
  ...
  while( counteval < budget){
    ...
    populationRepaired <- apply(population,2,ScaledMutant)
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(ScaledMutant(newMean)))
      ...
      cumMeanRepaired <-ScaledMutant(cumMean)
