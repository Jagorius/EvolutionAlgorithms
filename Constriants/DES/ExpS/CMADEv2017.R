CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  ExpS <- function(x){
    indxs = which( x < lower )
    x[ indxs ] = upper[indxs] - log(1+ runif(length(indxs))*( exp(upper[indxs] - lower[indxs]) - 1))
    indxs = which( x > upper )
    x[ indxs ] = lower[indxs] + log(1+ runif(length(indxs))*(exp( upper[indxs] - lower[indxs]) - 1))
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(ExpS(newMean)))
