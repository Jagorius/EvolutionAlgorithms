CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  Drawing <- function(x){
    indxs = which( x < lower )
    x[ indxs ] = runif(length(indxs), min=lower[indxs], max=upper[indxs])
    indxs = which( x > upper )
    x[ indxs ] = runif(length(indxs), min=lower[indxs], max=upper[indxs])
  ...
  while( counteval < budget){
    ...
    populationRepaired <- apply(population,2,Drawing)
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(Drawing(newMean)))
      ...
      cumMeanRepaired <-Drawing(cumMean)
