CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  MidBase <- function(x, baseVector){
    indxs = which( x < lower )
    x[ indxs] = (lower[indxs] + baseVector[indxs])/2
    indxs = which( x > upper )
    x[ indxs ] = (baseVector[indxs]+ upper[indxs] )/2
  ...
  while( counteval < budget){
    ...
    populationRepaired <- apply(population,2,MidBase,baseVector=newMean)
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(MidBase(newMean,oldMean)))
      ...
      cumMeanRepaired <-MidBase(cumMean,cumMean_old)
