CMADE <- function(par, fn, ..., lower, upper, control=list()) {
  ...
  MidTarget <- function(x, oldIndividuals){
    if(prevPopCounter == 0)
      oldIndividual = oldIndividuals
    else{
      oldIndividual = oldIndividuals[,prevPopCounter]
      prevPopCounter <- prevPopCounter + 1
    }
    indxs = which( x < lower )
    x[ indxs ] = (lower[indxs] + oldIndividual[indxs])/2
    indxs = which( x > upper )
    x[ indxs ] = (oldIndividual[indxs]+ upper[indxs] )/2
  ...
  while( counteval < budget){
    ...
    while (counteval < budget && !stoptol) {
      ...
      if (log.mean) mean.log <- rbind(mean.log,fn_l(MidTarget(newMean,oldMean)))
      ...
      prevPopCounter <- 0
      cumMeanRepaired <-MidTarget(cumMean,cumMean_old)
